...
public class VentaServiceImpl implements VentaService {
    ...
    public List<VentaDTO> listarVentas() {
        return ventaRepository.findAll().stream()
                .map(venta -> {
                    PedidoDTO pedido = pedidoClient.obtenerPedidoPorId(venta.getPedidoId());
                    normalizarPreciosPedido(pedido); // 💥 Enriquecé antes de construir el DTO
                    return new VentaDTO(venta, pedido);
                })
                .toList();
    }
    ...
    @Override
    public List<PedidoDTO> filtrarPedidosPorNombreCliente(String nombre) {
        List<PedidoDTO> todos = pedidoClient.obtenerTodosLosPedidos();

        return todos.stream()
                .filter(p -> p.getCliente() != null && p.getCliente().getNombre() != null)
                .filter(p -> p.getCliente().getNombre().toLowerCase().contains(nombre.toLowerCase()))
                .peek(this::normalizarPreciosPedido) // 💥 aquí se aplican los precios
                .toList();
    }
