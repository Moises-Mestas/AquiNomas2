...
import { MenuService } from '../../core/services/menu.services'; // Asegúrate de importar el servicio de menús
...
export class DetallePedidoPage {
  ...
  constructor(
    ...
    private menuService: MenuService  // Inyectamos el servicio de menús
  ) {}

  ngOnInit() {
    this.getDetallePedidos();
    this.getMenus(); // Cargamos los menús disponibles
  }
  ...
  getMenus() {
    this.menuService.getMenus().subscribe(
      (response) => {
        this.menus = response;  // Guardamos los menús disponibles en el arreglo
      },
  ...
  editDetallePedido(detallePedido: any) {
    ...
    this.newDetallePedido = {
      ...
      items: detallePedido.items.map((item: any) => ({
        menu_id: item.menu.id,  // Aquí asignamos el id del menú
        cantidad: item.cantidad
      }))
    };
    this.editing = true;
